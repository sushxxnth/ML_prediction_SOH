...
# Add project root and enforce working directory context
...
def verify_counterfactual() -> Dict:
    """
    Verify counterfactual optimizer claim (34.6% average mechanism reduction).
    Reads from counterfactual_validation_results.json generated by
    validate_counterfactual_optimization.py
    """
    print("\\n" + "="*70)
    ...
    if computed_reduction is None:
        print("❌ 'avg_mechanism_reduction_pct' key not found in results.")
        print(f"   Available keys: {list(summary.keys())}")
        return {'claim': 'Counterfactual Optimizer', 'verified': False}

    scenarios_tested = results.get('scenarios_tested', '?')
    print(f"\\nCounterfactual results (from {scenarios_tested} scenarios):")
    print(f"  Avg mechanism reduction: {computed_reduction:.1f}%")
    print(f"  Paper claim:             34.6%")

    # Allow ±5% tolerance for run-to-run variation
    ...
    results.append(verify_counterfactual())  # ← NEW
