...
colors = ['#26A69A', '#26A69A', '#26A69A', '#26A69A', '#26A69A', '#00796B']
...
for i, (acc, c, t) in enumerate(zip(accuracies, correct_counts, total_counts)):
    ...
    if i == len(datasets) - 1:  # Overall
        label = f'{acc:.1f}% ({c}/{t})'
    ax_a.text(acc + 1, y, label, va='center', fontsize=10, fontweight='bold' if i==len(datasets)-1 else 'normal')

ax_a.set_yticks(y_pos)
ax_a.set_yticklabels(datasets, fontsize=10)
# Make "Overall" bold (it's at position 0 because y_pos is reversed)
labels = ax_a.get_yticklabels()
labels[0].set_fontweight('bold')

ax_a.set_xlim(0, 115)
ax_a.set_xlabel('Accuracy (%)', fontsize=10)
...
ax_b.set_xticklabels(labels, fontsize=8, rotation=0)
ax_b.set_yticklabels(labels, fontsize=8)
ax_b.set_xlabel('True', fontsize=10, fontweight='bold')
ax_b.set_ylabel('Predicted', fontsize=10, fontweight='bold')
...
bar_colors = ['#26A69A', '#2E7D32', '#26A69A', '#2E7D32']
...
ax_c.text(3.5, 81, 'Benchmark\nThreshold', fontsize=7, ha='center', color='#666666')

ax_c.set_ylim(0, 110)
ax_c.set_ylabel('Recall (%)', fontsize=10)
...
bars1 = ax_d.bar(x, overall, bar_width*2, color=['#4DB6AC', '#26A69A', '#00796B'],
...
ax_d.text(1.5, 88, '+18.7 pp', fontsize=9, fontweight='bold', color='#C44E52', 
          ha='center', rotation=30)

ax_d.set_xticks(x)
ax_d.set_xticklabels(architectures, fontsize=9)
ax_d.set_ylabel('Performance (%)', fontsize=10)
ax_d.set_ylim(0, 110)
ax_d.spines['top'].set_visible(False)
ax_d.spines['right'].set_visible(False)

plt.savefig('Casual_Attribution_reports/pinn_validation_92.png',
            dpi=150, bbox_inches='tight', facecolor='white')
print("✅ Saved pinn_validation_92.png")
plt.close()
